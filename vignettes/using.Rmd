...
Add a section, presumably at the start of the document, that authorizes the tutorial to access the user-credential information and the log storage. For Google sheets storage, the section will 
look like this.
...
It's easy to provide another way to access the submissions, which might be convenient if there are multiple graders. `{Submitr}` provides a means to add a submissions-download link to the tutorial in a secure way. To do this, add the following two chunks to your tutorial:

    `` `{r echo=FALSE}
    secret_word <- "something_you_can_remember"
    `` `

    `` `{r child = system.file("download_link.Rmd", package = "submitr")}
    `` `
The chunks must be distinct; you can't combine the content into a single chunk.

These two chunks will cause to be displayed an empty text-entry box labeled "For grading only." When the secret word is typed into that box, a link will appear for downloading the submissions file by the browser.

I imagine the main use for this will be for instructors and graders, to give them access to the stored submissions for each tutorial. However, by giving students the secret word (perhaps something obvious like `download`), students can access the submissions as well. Allowing this would make sense if students are running the tutorial on their own computers or using `rstudio.cloud` or a similar server. In this scenario, students would hand in their submissions file by, e.g., uploading it to a course support system such as Blackboard or Moodle. 
